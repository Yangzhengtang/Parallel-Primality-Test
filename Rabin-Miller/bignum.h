...
static mpz_t a_array[MAX_ROUND];
static mpz_t x_array[MAX_ROUND];
#define THREAD_NUM 8
#define SEQ_PART 64
...
//  Sequential Component
    ...
    mpz_init(la);
    mpz_init(lx);
    for(int i=0; i<k/SEQ_PART; i++){    //  [Par-Opt]   This for loop can be performed in parallel
...
int bignum_rabin_miller_para_ttt(mpz_t test_num, int k){
    if(mpz_cmp_ui(test_num, 2) < 0)
        return 0;
    if(mpz_cmp_ui(test_num, 2) == 0)   
        return 1;
    ...
    for(int i=0; i<k; i++){
        ...
        if(!early_terminate){
            //  printf("can't early return\n");
            bigNumIsPrime[i] = 0;
            //  printf("can't early return 1\n");
            shouldStop = 1;
            //  printf("can't early return 2\n");
        }  
        else{
            //  printf("can early return\n");
            bigNumIsPrime[i] = 1;
        } 
    }
